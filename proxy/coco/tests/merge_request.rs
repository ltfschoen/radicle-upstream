...
async fn can_fetch_project_changes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tag_id = repo
        .tag("MY_TAG", &head_commit, &alice_signature, "message", false)
        .unwrap();
    repo.reference("refs/heads/MY_REF", tag_id, false, "")
        .unwrap();
    ...
    let bob_monorepo = git2::Repository::open(bob_state.monorepo())?;
    let refs = bob_monorepo
        .references_glob(&format!(
            "refs/namespaces/{}/refs/remotes/*",
            coco::git::types::Namespace::from(project.urn())
        ))
        .unwrap();
    let refs = refs
    ...
    assert!(refs.iter().any(|r| r.contains("MY_REF")));
    assert!(refs.iter().any(|r| r.contains("MY_TAG")));
